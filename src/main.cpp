...
#if defined(NDEBUG)
#error "CarebitCoin cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCARE spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zCARE spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zCARE spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight < 400) {
          nSubsidy = 50000 * COIN;
    } else if (nHeight <= 7000 && nHeight >= 400) {
          nSubsidy = 700 * COIN;
    } else if (nHeight <= 32000 && nHeight > 7000) {
          nSubsidy = 600 * COIN;
  	} else if (nHeight <= 82000 && nHeight > 32000) {
          nSubsidy = 500 * COIN;
  	} else if (nHeight <= UPD1_BLOCK && nHeight > 82000) {
          nSubsidy = 400 * COIN;
  	} else if (nHeight <= UPD2_BLOCK && nHeight > UPD1_BLOCK) {
          nSubsidy = 200 * COIN;
  	} else if (nHeight <= 300000 && nHeight > UPD2_BLOCK) {
          nSubsidy = 100 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from CarebitCoin
...
{
    RenameThread("carebitcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZCAREBITCOINMinted()
{
...
void RecalculateZCAREBITCOINSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
        ...
        //Remove spends from zCARE supply
...
bool RecalculateCAREBITCOINSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zCARE with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zCARESpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert CAREBITCOIN to zCARE
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zCARE spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zCARE serial %s in block\n Block: %s",
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          if (nHeight > UPD2_BLOCK ) {
            ...
            if (block.vtx[1].vout[s - 1].scriptPubKey != CreateOpFundScriptPubKey())
                return state.DoS(100, error("CheckBlock() : incorrect Op fund payment"));
            if (block.vtx[1].vout[s - 2].scriptPubKey != CreateEmFundScriptPubKey())
                return state.DoS(100, error("CheckBlock() : incorrect Em fund payment"));
          }
          ...
          // Inputs
          std::vector<CTxIn> careInputs;
          std::vector<CTxIn> zCAREInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zCAREInputs.push_back(stakeIn);
              }else{
                  careInputs.push_back(stakeIn);
              }
          }
          const bool hasCAREInputs = !careInputs.empty();
          const bool hasZCAREInputs = !zCAREInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasCAREInputs)
                      ...
                      for (CTxIn careIn : careInputs){
                          if(careIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
              ...
              // Now that this loop if completed. Check if we have zCARE inputs.
              if(hasZCAREInputs){
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zCARE mints and %d zCARE spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CarebitCoin: We use certain sporks during IBD, so check to see if they are
