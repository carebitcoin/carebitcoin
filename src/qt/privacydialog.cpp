...
#include "zcarebitcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCARE ought to be enough for anybody." - Bill Gates, 2017
    ui->zCAREpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCARESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCARE->setEnabled(false);
        ui->pushButtonMintzCARE->setToolTip(tr("zCARE is currently disabled due to maintenance."));

        ui->pushButtonSpendzCARE->setEnabled(false);
        ui->pushButtonSpendzCARE->setToolTip(tr("zCARE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCAREpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCARE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCARE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCARE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCARE in ") +
...
void PrivacyDialog::on_pushButtonSpendzCARE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCARE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCARE
        sendzCARE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCARE
    sendzCARE();
}

void PrivacyDialog::on_pushButtonZCarebitCoinControl_clicked()
{
    ZCarebitCoinControlDialog* zCAREControl = new ZCarebitCoinControlDialog(this);
    zCAREControl->setModel(walletModel);
    zCAREControl->exec();
}

void PrivacyDialog::setZCarebitCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCARESelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCARE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid CarebitCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCAREpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCARE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CAREBITCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCARE</b>";
    ...
    // use mints from zCARE selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCarebitCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCarebitCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcarebitcoin selector in case it was used
    ZCarebitCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCARE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " CarebitCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCARE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCARE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCARE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCARE "));
    ui->labelzCAREAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCARESyncStatus->setVisible(fShow);
}
