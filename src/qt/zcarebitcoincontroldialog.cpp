...
#include "ui_zcarebitcoincontroldialog.h"
...
std::list<std::string> ZCarebitCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCarebitCoinControlDialog::listMints;

ZCarebitCoinControlDialog::ZCarebitCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCarebitCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCarebitCoinControlDialog::~ZCarebitCoinControlDialog()
{
    delete ui;
}

void ZCarebitCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCarebitCoinControlDialog::updateList()
{
...
void ZCarebitCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCarebitCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCarebitCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCarebitCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCarebitCoinControlDialog::GetSelectedMints()
{
...
void ZCarebitCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
